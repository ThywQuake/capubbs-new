...
export function saveStoredReplyDraft(
  ...
  const updatedDraft: StoredReplyDraft = {
    ...draft,
    id: draft.id || createReplyDraftId(draft.threadId),
  ...
  writeStoredReplyDrafts(storageKey, nextDrafts);
  notifyReplyDraftChange(storageKey);

  return updatedDraft;
}

export function deleteStoredReplyDraft(draftId: string, ownerKey: string | null | undefined) {
  const storageKey = getReplyDraftStorageKey(ownerKey);
  ...
  const storedDrafts = readStoredReplyDrafts(ownerKey);
  const nextDrafts = storedDrafts.filter((storedDraft) => storedDraft.id !== draftId);

  if (nextDrafts.length === storedDrafts.length) {
    return false;
  }
...
function writeStoredReplyDrafts(storageKey: string, drafts: StoredReplyDraft[]) {
  if (typeof window === 'undefined') {
    return;
  }
  ...
  try {
    ...
    // Drafts are a convenience layer; failed writes should not interrupt editing.
  }
}
...
function createReplyDraftId(threadId: string) {
  return `reply-${threadId}-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`;
}

function isStoredReplyDraft(value: unknown): value is StoredReplyDraft {
  if (!isObjectRecord(value) || !isStoredReplyEditorValue(value.editor)) {
    return false;
  }
