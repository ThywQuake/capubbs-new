export const activityGradientPresets = [
  'from-red-500 via-rose-500 to-pink-500',
  'from-orange-500 via-amber-500 to-yellow-400',
  'from-yellow-400 via-lime-500 to-green-600',
  'from-emerald-500 via-teal-500 to-cyan-500',
  'from-cyan-400 via-sky-500 to-blue-600',
  'from-blue-500 via-indigo-500 to-violet-600',
  'from-violet-500 via-fuchsia-500 to-pink-500',
  'from-slate-600 via-zinc-500 to-stone-400',
] as const;

function md5Hex(input: string) {
  const words = stringToWordArray(new TextEncoder().encode(input.normalize('NFKC').trim()));
  let a = 0x67452301;
  let b = 0xefcdab89;
  let c = 0x98badcfe;
  let d = 0x10325476;
...
function pickActivityGradientIndex(title: string) {
  const digest = md5Hex(title.trim());
  return Number.parseInt(digest.slice(0, 8), 16) % activityGradientPresets.length;
}

export function getActivityGradientClass(title: string) {
  return activityGradientPresets[pickActivityGradientIndex(title)];
}

function stringToWordArray(bytes: Uint8Array) {
