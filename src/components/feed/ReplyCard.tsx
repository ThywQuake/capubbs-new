...
import type { ReplyItem } from '../../types/forum';
...
export function ReplyCard({ compact = false, reply }: ReplyCardProps) {
  const navigate = useNavigate();
  const threadPath = getThreadPathFromHref(reply.href);
  const openThread = () => {
    navigate(threadPath);
  };
    ...
    return (
      ...
      >
        ...
        <p className={compactFeedMetaClassName}>
          ...
          <span className={compactFeedTimeClassName}>{formatPostTimestamp(reply.time)}</span>
        </p>
      </article>
    );
  }
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-4">
        <UserBlock id={reply.id} rating={reply.rating} href={reply.authorHref} />
        <Link
          to={getBoardPath(reply.board)}
          onClick={(event) => event.stopPropagation()}
      ...
      <div className="mt-4 flex items-center gap-4 text-sm leading-[var(--capubbs-thread-card-line-height)] text-[#875A41] dark:text-white">
        <span>回复时间：{formatPostTimestamp(reply.time)}</span>
        <Link
          to={threadPath}
          onClick={(event) => event.stopPropagation()}
